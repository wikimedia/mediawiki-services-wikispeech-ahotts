    ...
    def do_POST(self):
        if "ahotts_getaudio" in self.path:
            if self.rfile:
                ...
                #python3 requires encode here
                jsonstr = json.dumps({'wav':"tts_%s.wav" % (hashnumber),'wrd':"tts_%s.wrd" % (hashnumber)}).encode()
...
if __name__ == "__main__":
    try:
        address=sys.argv[1]
        port=sys.argv[2]
        print('starting server %s %s' % (sys.argv[1],sys.argv[2]))
